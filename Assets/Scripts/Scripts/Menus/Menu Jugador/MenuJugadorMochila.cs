...
public class MenuJugadorMochila : MonoBehaviour
{
    ...
    public void prepararMenuMochila(GameObject plantillaItem) {
        ...
        limpiarItemsMochila(contentListMochila); //Necesario sino salen duplicados los items
    ...
    public void clickButtonUsarItem(GameObject interfazItem) {
        ...
        for (int i = 0; i < jugador.EquipoPokemon.Count; i++) {
            ...
            if (pokemon.HP < pokemon.HPMaximos)
            {
                ...
                interfazPokemon.GetComponentsInChildren<Image>()[4].sprite = Utilidades.convertirArrayBytesASprite((pokemon.ImagenDeFrente != null) ? pokemon.ImagenDeFrente : pokemon.ImagenDeEspalda);
    ...
    private void limpiarItemsMochila(GameObject contentScrollView) {
