...
public class ScriptPlayer : MonoBehaviour
{
    ...
    void Start()
    {
        if (DatosGenerales.ultimaEscena != null && !DatosGenerales.ultimaEscena.Equals("LobbyScene")) {
            Transform posicionJugador = GetComponent<Transform>();
            switch (DatosGenerales.ultimaEscena) {
                case "AdventureZone":
                    posicionJugador.position = new Vector2(-1, 5);
                    break;

                case "PokemonCenterScene":
                    posicionJugador.position = new Vector2(4.5f, -1f);
                    break;

                case "PokemonShopScene":
                    posicionJugador.position = new Vector2(-6.84f, -1f);
                    break;
                default:
                    posicionJugador.position = new Vector2(-1f, -1f);
                    break;
            }
        }
